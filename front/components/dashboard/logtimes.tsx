...
const Logtimes = () => {
  ...
  const getChartData = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const log = logtimes.find(log => dayjs(log.logtime_day).isSame(date, "day"));
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => setWeekStart(prev => prev.subtract(7, "day"))}>
          <Text style={styles.arrow}>◀</Text>
        </TouchableOpacity>

        <Text style={styles.date}>
          Semaine du {weekStart.format("DD/MM/YYYY")} • Total: {Math.floor(chartData.total / 60)}h{(chartData.total % 60).toString().padStart(2, "0")}
        </Text>

        <TouchableOpacity onPress={() => setWeekStart(prev => prev.add(7, "day"))}>
          <Text style={styles.arrow}>▶</Text>
        </TouchableOpacity>
      </View>
