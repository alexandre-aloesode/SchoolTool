...
const JobsInProgress = () => {
  ...
  const getProgressInfo = (job) => {
    ...
    const delay = Math.max(0, -remaining); // positive if overdue
      ...
      <View style={styles.row}>
        ...
        <View style={[styles.progressContainer, { flex: 3 }]}>
          <View style={styles.barAndLabel}>
            <ProgressBar progress={progress} color="#0097a7" style={styles.progress} />
            {isLate && (
              <Text style={styles.lateLabel}>Retard de {delay} jours</Text>
            )}
          </View>
          <View style={styles.iconGroup}>
            <IconButton icon="clock-outline" size={18} onPress={() => {}} />
